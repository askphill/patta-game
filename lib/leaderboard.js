...
export const TOP_TEN_CACHE_KEY = 'leaderboard:top20_cache';
...
async function fetchTopFromRedis(count = 20) {
...
export async function rebuildAndCacheTopTen(count = 20) {
  const entries = await fetchTopFromRedis(count);
  await redis.set(TOP_TEN_CACHE_KEY, JSON.stringify(entries), { ex: TOP_TEN_CACHE_TTL });
  return entries;
}

export async function getCachedTopTen(count = 20) {
